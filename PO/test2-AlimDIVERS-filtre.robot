...
filtrer les produits by attribute
                    [Arguments]          ${Courant_sortie_Max}        ${Puissance}          ${Taille_Dime}       ${Tension_d'entrée_Max}     ${Tension_d_entrée_Min}
                    select from list by index          ${Courant_sortie_MaxDiv}          ${Courant_sortie_Max}
                    sleep       5
                    select from list by index          ${Puissance_Divers}       ${Puissance}
                    sleep       5
                    select from list by index          ${Taille_Dime}       ${Taille_Dime}
                    sleep       5
